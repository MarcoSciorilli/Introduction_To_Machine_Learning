...
def downloadFoodTweets(api, users, count, food_dict, n_percentiles = 4):
    ''' downloads number 'count' tweets from users listed in 'companies' 
        ...
        count: maximum number of tweets to be downloaded from a single account
        food_dict: food dictionary needed to detect food-related tweet
        n_percentiles: number of percetiles used to create the popularity_index

        Returns
        -------
        data: dataframe of containig tweets and some of their properties
        popularity_index: index of popularity of the tweets
    ...
    data = DataFrame(columns=['text','favorite_count', 'retweet_count', 'replies_count', 'popularity',
    ...
    for user in users:
        ...
        # loop through tweets
        for tweet in api.user_timeline(screen_name = user, count = count):

            # tweets must not be retweeted tweets and must not be replies
            if (not tweet.retweeted) and ('RT @' not in tweet.text)  and (not tweet.in_reply_to_status_id_str != None) and tweet.user.followers_count >= 1e6:

                # get decoded tweet's text
                tweets_encoded = tweet.text.encode('utf-8')
                tweets_decoded = tweets_encoded.decode('utf-8')

                # check for words of food dict
                # text_tokens = word_tokenize(tweets_decoded.lower())
                # tokens_without_sw = [word for word in text_tokens if not word in all_stopwords]
                # n_words_in_food_dict = 0
                # for word in tokens_without_sw:
                #     if word in food_dict:
                #         n_words_in_food_dict += 1
                # if n_words_in_food_dict == 0:
                #     continue
                ...
                if  foodtweet_cleaner(food_dict, tweets_decoded):
                    ...
                    data.loc[n_row] = [tweets_decoded,
                                       tweet.favorite_count, tweet.retweet_count, replies_count, tweet.favorite_count + tweet.retweet_count,
    ...
    # data['time'], day = factorizeDates(data['time'])
    # data.insert(data.columns.get_loc('time'), 'day', day)
